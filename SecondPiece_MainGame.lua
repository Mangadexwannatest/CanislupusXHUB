...
do
    ...
    local BossOption = Tabs.Main:AddDropdown("BossSelect", {
        Title = "Select Boss",
        Values = {"Cid","Kashimo","Sukuna","Natsu","Artoria","Gojo"},
        Multi = true,
        Default = {nil},
    })
    ...
    BossOption:OnChanged(function(Value)
        Boss_Main = {}
        for Value, State in next, Value do
            table.insert(Boss_Main, Value) 
        end
    end)
    ...
    Tabs.Main:AddToggle("AutoBossSpawn", {Title = "Auto Select Boss", Default = false })
...
coroutine.resume(coroutine.create(function()
    while wait() do pcall(function ()
        if not Options.AutoMining.Value or getgenv().STOP or CheckBossSpawn() or getgenv().InMerchant or Chestawd() or getgenv().InBossHunt 
        ...
        if Chestawd() or CheckBossSpawn() or getgenv().InBossHunt or not getgenv().QuestHuntOnCoolDown or getgenv().STOP or not getgenv().InMining then
            if not getgenv().InMining and game.ReplicatedStorage.Settings[game.Players.LocalPlayer.Name]:FindFirstChild("Mining") then
                game:GetService("Players").LocalPlayer.Character.InputHandle.Mining:FireServer()
            end
        end
   end)
...
game.ReplicatedStorage.Settings.ChildAdded:Connect(function(v)
    ...
    if v.Name == CriminalCheck().Name and Options.Dodge_Skill_BountyHunt.Value then
        ConnectCriminal = game.ReplicatedStorage.Settings[v.Name].ChildAdded:Connect(function(c)
            if c.Name == "Action" or c.Name == "IFrame" and Options.Dodge_Skill_BountyHunt.Value then
                getgenv().Dodge_Criminal = true
...
game.ReplicatedStorage.Settings[game.Players.LocalPlayer.Name].ChildAdded:Connect(function (v)
    pcall(function ()
    if v.Name == "Action" or v.Name == "IFrame" and Options.Dodge_Skill_BountyHunt.Value then
...
while wait() do pcall(function ()
    ...
    coroutine.resume(coroutine.create(function()
        ...
        if Options.AutoBuyMerchantItem.Value and game.PlaceId == 15049111150 and DistOfMerchant() <= 20 and not getgenv().AlrBuy then
