   ...
   local function VisibleForCodex(value)
    ...
    local function check_the_unitspawns(Unit,Position)
        ...
        for i,v in pairs(workspace:WaitForChild("Unit"):GetChildren()) do
            ...
            if v:WaitForChild("HumanoidRootPart").Position == Position or (v:WaitForChild("HumanoidRootPart").Position - Position).magnitude < 2 then
                return v
            end
            end
        end
    ...
    local remoteinput = game:GetService("ReplicatedStorage").Remotes.Input
    local remoteserver = game:GetService("ReplicatedStorage").Remotes.Server
                ...
                for val = 1,#getgenv().Playing do
                    for i,v in pairs(getgenv().Playing[val]) do
                        ...
                        if i == "VoteGameMode" and game.Players.LocalPlayer.PlayerGui.HUD.ModeVoteFrame.Visible and not Options.Auto_Vote.Value then
                            ...
                            wait_for(v)
                            remoteinput:FireServer(tostring(i),tostring(v["Value"]))
                        ...
                        elseif i == "SpeedChange" then
                            ...
                            wait_for(v)
                            remoteinput:FireServer(tostring(i),v["Value"])
                        ...
                        elseif i == "UseSpecialMove" then
                            ...
                            current_play_action["Action"] = {
                                ...
                                wait_for(v)
                                repeat
                                remoteinput:FireServer(tostring(i),check_the_unitspawns(v["Unit"],v["Position"]),tostring(v["Value"]))
                                task.wait(0.25)
